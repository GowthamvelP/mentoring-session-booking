...
RSpec.describe BookingCancellationJob, type: :job do
  ...
  let(:booking) { create(:booking, :cancelled, slot: slot, member: member, organization: organization) }

  before { ActsAsTenant.current_tenant = organization }
  after { ActsAsTenant.current_tenant = nil }

  it "executes without error and logs the cancellation event" do
